...
> ⚠️ 배 신화·원황 점수는 이후 **63.0 · 49.1**로 내려갔다. 자료가 두 품종의 수확기를
> 단일 날짜로만 적어 두어 수확기 기상을 **하루치 평년값**으로 채점하고 있었다 — §24.3.
...
## 24. 감자·사과·배 적합도 점수의 이유·근거 (2026-08-05)

§23에서 상추·오이 작형 카드에 "채점 축마다 수치·비교기준·출처 한 줄"을 붙였다. 이 절은
같은 것을 감자·사과·배에 맞춘다. **먼저 재보니 상태가 서로 달랐다.**

| 작물 | 채점 축 | 근거 문장(이전) | 문제 |
|---|---|---|---|
| 감자 | 7개 | pros 4 / cons 4 | 축 7개 중 **재배기간 1개만** 문장이 있었다. 비대온도 65.1점처럼 점수를 깎은 항목이 화면에 아무 흔적도 남기지 않았다 |
| 사과 | 6개 | **pros 0 / cons 0** | 근거가 아예 없었다 |
| 배 | 4개 | **pros 0 / cons 0** | 근거가 아예 없었다 |

### 24.1 사과·배는 근거와 역병이 **사라져 있었다** (회귀)

§22에서 사과·배를 조건 모드(`cultivar_conditions`)에서 기후 모델(`cultivar_fruit_fit`)로
옮길 때 `cultivar_reasons.build` 호출과 `blight_data.blight_info` 호출을 함께 옮기지 않았다.
그래서 카드가 머리줄만 남고, 79.2점이 어디서 나왔는지 화면에 한 줄도 없었다.
**역병 안내도 같이 사라졌다** — 사과 홍로는 위험도가 실제로 조사된 4품종 중 하나다.

§22를 검증할 때 점수·순위·응답 계약(필수 10필드)은 봤지만 **근거 줄 수를 세지 않았다.**
숫자가 맞으면 통과라고 본 것이 원인이다. 이번에는 `pros == 0 and cons == 0`을 검사에 넣었다.

### 24.2 축마다 한 줄 — `_axis_reasons`

`cultivar_fit._axis_reasons` (감자 7축) · `cultivar_fruit_fit._axis_reasons` (과수 6축).
문장은 **(측정값 · 비교기준 · 판정)**, basis는 **(관측소 평년 N년 · 기준의 출처)**다.

```
✅ 비대기 평균기온이 20.0℃로 이 품종의 비대 적온(14~18℃)에 들어요
    └ 천안 평년 10년 · 품종자료 비대적온
⚠️ 착색기(수확 전 30일) 평균기온이 25.1℃로 착색 적온(12~13℃)보다 높아 색이 덜 들 수 있어요
    └ 청송군 평년 10년 · 품종자료 coloring_daily_mean_c
```

basis에 관측소와 연수를 적는다. 이전에는 전부 `지역 기상`이라 어느 관측소의 몇 년치인지
확인할 방법이 없었다.

`cultivar_reasons`의 상한을 4 → **7**로 올리고, 지역 근거가 차지할 자리를 **5줄로 제한**
(`MAX_REGION`)했다. 4줄이면 축 문장이 앞자리를 다 먹어 품종 특성 문장(선택조건·초보적합·
저장성)이 한 줄도 못 나갔다 — "왜 이 점수인가"와 "이 품종은 어떤 품종인가"는 둘 다 필요하다.
카드가 접혀 있으므로(§23.9) 펼쳤을 때 길어지는 것은 문제가 아니다.

### 24.3 배 신화·원황 — 수확기가 **하루**로 채점되고 있었다

근거 문장을 붙이자 바로 드러났다: **"수확기 강수가 평년 5mm(하루평균 5.0mm)로 많아
열과와 수확 지연에 주의해야 해요"** — 총 5mm를 '많다'고 말한다.

원인은 자료다. 배 수확기 서술이 품종마다 형식이 다르다.

| 품종 | 원문 | 해석 |
|---|---|---|
| 신고 | 9월 하순~10월 상·중순 | 09-21~10-20 (30일) |
| 원황 | **9월 1일** | 09-01~09-01 (1일) |
| 신화 | **9월 15일** | 09-15~09-15 (1일) |

단일 날짜를 1일 구간으로 채점하면 **가중치 36%가 걸린 수확기 강수가 그 하루의 평년값**에
좌우된다. 길이를 우리가 만들지 않고, **같은 자료가 범위를 준 품종들의 중앙값**을 써서
적숙기 날짜를 중심으로 넓혔다(`_typical_harvest_span`·`_widen_harvest`). 그 사실은 cons에
그대로 밝힌다.

| | 이전(1일 창) | 이후(30일 창) |
|---|---|---|
| 신화 | 82.4 | **63.0** (강수 172mm · 30℃ 초과 6.4일) |
| 원황 | 81.9 | **49.1** (강수 228mm · 30℃ 초과 14.4일) |

원황 채점구간은 08-17~09-15다. 나주 8월 하순은 늦더위와 장마 끝이므로 새 점수가 실제에
가깝다. 순위(신고 > 신화 > 원황)는 그대로다.

**화면·농업일지의 수확기는 자료 원문 그대로 둔다.** 넓힌 구간을 `harvest_window`에 넣으면
일지의 수확 단계가 자료에 없는 날짜로 옮겨진다(원황 '9월 1일' → 08/22~09/11). 채점에 쓴
구간은 `scored_window`로 따로 싣는다.

### 24.4 원황에 조생종 안내가 안 붙던 문제

§22.4는 "조기 출하용 품종은 구조적으로 낮게 나온다"는 사실을 배지·주의문으로 밝히기로
했는데, 배 세 품종 모두 `early_market_preferred=False`였다. `category`·`primary_use`·
`market_use`가 전부 비어 있고 조기 출하 성격이 **`selection_conditions` 문장에만** 적혀
있었기 때문이다(원황 '남부 조기 출하 또는 중부 추석 출하를 계획하는 경우').
`_early_market_preferred`가 그 필드도 읽게 했다. 사과 쓰가루와 같은 함정이었다.

문구도 작물에 맞췄다. 배는 착색 두 항목이 자료 부재로 **제외**되므로 "이 점수는 착색을
크게 보는데"가 사실이 아니다 — 배에서 조생종을 깎는 것은 수확기가 늦더위·장마 끝에
걸리는 것이다.

### 24.5 `cautions`가 화면에 닿지 않고 있었다

과수 모델은 조기 출하 안내와 만개후일수 추정치 안내를 `cautions`에 넣었는데, **품종 카드는
`cautions`를 렌더하지 않는다**(§18에서 cons와 중복이라 지웠다). 그래서 §22.4의 핵심 안내가
페이로드에만 남아 있었다. 이제 `region_cons`로 함께 넘긴다.

순서도 정했다. 챗봇은 '고려할점'을 3줄만 쓰므로:
- **맨 앞** — 점수를 *읽는 방법*을 바꾸는 안내(조기 출하용 품종)
- 가운데 — 채점 축 근거
- **맨 뒤** — 자료 신뢰도 각주(만개후일수 추정치)

각주를 앞에 두었을 때 후지의 3줄이 각주 + 착색 2줄이 되어 '첫서리' 정보가 밀려 나갔다.

### 24.6 조사 오류 두 건

- `토양 pH이 5.79로` → `with_subject()`(받침에 맞는 이/가)를 추가. 'pH'처럼 한글이 아닌
  낱말은 '가'로 읽는다("pH가 5.8이에요").
- `수확이 06/16로` → 숫자에 조사를 붙이지 않는다. '십육일로'인지 '십육으로'인지는 읽는
  방식에 달려 규칙으로 정할 수 없어, 괄호로 뺐다(`수확 시기가 조기 출하 구간에 들어와요
  (예상 수확 06/16)`).

### 24.7 감홍 "첫서리까지 0일 여유가 있어요"

여유 0일을 장점으로 올렸다(감홍 수확종료 10/20 = 첫서리 10/20). 14일 이상만 장점으로 두고,
0~13일은 "늦게 따는 몫은 서리를 맞을 수 있어요"로, 음수는 **"성목이 죽는다는 뜻은
아니지만"**을 붙여 cons로 옮겼다 — 초보자가 "수확이 첫서리보다 21일 늦다"만 읽으면
나무가 죽는다고 생각한다(첫서리는 일 최저 0℃ 첫날이고 성목 내한성은 훨씬 아래다).

### 24.8 검증

| 항목 | 결과 |
|---|---|
| 감자 채점 기준선 | 변경 전 스냅샷과 **바이트 동일** (점수·파종일·항목점수 불변) |
| 사과 점수 | 79.2 / 66.1 / 58.5 / 46.0 / 40.6 — §22와 동일 |
| 배 점수 | 신고 89.3 불변 · 신화 63.0 · 원황 49.1 (§24.3의 이유로 변경) |
| 근거 0줄 품종 | **0건** (이전 사과 5품종·배 3품종 전부 0줄) |
| basis 없는 근거 줄 | 0건 |
| 역병 안내 | 감자 4품종·사과 홍로 등급 표시 복구 |
| 조기 출하용 배지 | 사과 쓰가루 · 배 원황 |
| 화면 | 사과 카드 접힘 6줄 → 펼침 25줄(근거 13건), 페이지 오류 0건 |
| 챗봇 | 사과·배 추천이유·고려할점·역병 전달 확인 |
