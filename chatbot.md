...
## 16. 구현 결과 4차 — 상추·오이 작형 순위 대응 (2026-08-05)

`breed.md §23`으로 상추·오이의 `/api/cultivar-score` 응답이 **품종 순위에서 작형 순위로**
바뀌었다. 챗봇 쪽에서 이걸 그대로 흘리면 모델이 작형 이름을 품종 이름으로 읽는다.

### 16.1 도구 응답의 키를 갈랐다

`tool_get_cultivar_candidates`는 `scoring_mode == "season"`이면 목록을 `품종`이 아니라
**`작형`** 키에 담고, `순위단위`를 함께 싣는다. 작형 항목에는 `재배구조`(노지/시설/겸용)와
**`이작형에쓸품종`**(자료 근거 포함)이 붙는다.

```
{"순위단위": "작형",
 "작형": [{"이름": "억제재배", "점수": 90.6, "재배구조": "노지·시설 겸용",
          "이작형에쓸품종": [{"이름": "가시오이", "근거": "고온 적응성이 …"}]}, …],
 "점수의뜻": "점수는 그 작기 동안 바깥(노지) 기상이 …",
 "작형순위인이유": "품종이 아니라 '작형'을 줄 세웠어요. …"}
```

`이지역에서제외된작형`도 이유와 함께 실어, 모델이 "고랭지재배는 왜 없나"에 답할 수 있게 했다.

### 16.2 SYSTEM_PROMPT 규칙 (`# 작형(재배 시기) 순위로 답하는 작물 — 상추·오이`)

- 목록 키가 `작형`이면 그 `이름`은 **품종이 아니다**. "여름재배 품종을 추천합니다"는 틀렸다.
- 상추·오이에서 품종 이름을 말할 수 있는 유일한 출처는 `이작형에쓸품종`이다.
- "어떤 품종이 좋아요?"에는 (1) 작형을 점수와 함께 (2) 그 작형에 쓸 품종을 (3) 품종끼리는
  자료에 기후 수치가 없어 점수로 가릴 수 없다는 사실을 밝혀 답한다. **없는 순위를 만들지 않는다.**
- 시설 작형의 낮은 점수를 "재배 불가"로 말하지 않고 "난방·보온으로 메워야 하는 몫"으로 말한다.
- `재배구조`가 시설이면 하우스가 필요하다는 사실을 점수보다 먼저 말한다.

도구 설명도 고쳤다 — 감자·사과·배는 품종 순위, 상추·오이는 작형 순위임을 명시하고
`순위단위`로 확인하라고 적었다.
...
| 질문 | 답변 요지 |
|---|---|
| "천안에서 상추 키우려는데 어떤 품종이 좋아요?" | "상추 품종끼리 순위를 매길 자료가 없어요 — 품종별 기후 수치가 없어서 대신 작형으로 갈립니다. 노지 여름재배가 92.1점…" + 세 품종 제시 + 가을재배 52일에 로메인 60~90일은 부족 |
| "천안 오이는 어떤 작형이 제일 나아요?" | 억제재배 90.6점(우수), 노지·시설 겸용이라 초기 노지→늦추위 보온 |
...
| 상수 | 값 | 뜻 |
|---|---|---|
| `TOOL_BUDGET_SEC` | 20초 | 도구 **하나**에 허용할 시간 |
| `TURN_TOOL_BUDGET_SEC` | 30초 | 한 **턴**에서 도구에 쓸 수 있는 총합 |

> ⚠️ 처음에는 도구 하나에만 20초를 걸었다가 배포에서 그대로 실패했다. 조회가 실패하면
> 모델이 다음 라운드에서 **같은 도구를 다시 부른다**. 20초 × 4라운드(`MAX_TOOL_ROUNDS`)
> = 80초라 결국 60초를 넘겨 504가 그대로 났다(예열 상태인데도 60.3초).
> 턴 예산을 다 쓰면 이후 도구는 **기다리지 않고 즉시** 실패를 돌려 모델이 답을 맺게 한다.
> 실측: 영원히 멈추는 도구로 4라운드를 돌려도 20 → 10 → 0 → 0초, 총합 정확히 30초.

넘기면 모델이 '조회 실패'를 받아 사정을 설명하고, 그 사이 캐시가 채워져 다음 질문은
정상 응답한다.
