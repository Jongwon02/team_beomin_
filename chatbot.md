...
## 18. 응답 지연·504 후속 조치 + 대화 초기화 버튼 (2026-08-05)

§17로 도구 왕복은 없앴지만 사용자 체감은 여전히 "504가 나고, 답이 너무 늦다"였다.
남은 원인 둘을 고쳤다.

### 18.1 함수 예산 60초 → 300초

`vercel.json`의 `maxDuration: 60`은 예전 플랫폼 기본값이다. 지금 기본은 300초다.
콜드 스타트(약 33초) + 도구 예산(30초) + 모델(5~10초)을 다 더해도 60초에 아슬아슬하게
걸렸고, 그래서 콜드일 때만 504가 났다(예열되면 5~8초). 300초로 올려 **504 자체를 없앤다**.

임포트 비용은 코드가 아니라 플랫폼 몫이다(실측: 로컬 임포트 합계 1.7초 — anthropic 1.08,
crop_score_server 0.59, 나머지 0.00).

### 18.2 답변이 다 끝나야 화면에 뜨던 문제 — 스트리밍 복원

`api/chat.py`가 SSE 프레임을 **전부 모아 마지막에 한 번에** 보내고 있었다. 30초짜리 답이면
30초 동안 빈 화면이다. 로컬(8003)은 첫 토큰이 2~3초에 도착하는데 배포만 그렇지 않았던
이유가 이것이다.

`Content-Length`를 빼고 프레임마다 `flush()` 하도록 바꿨다. 런타임이 전송 인코딩을 알아서
처리한다.

> ⚠️ 직접 chunk 헤더(크기 + CRLF)를 써 넣으면 응답이 깨진다 — 그건 런타임의 몫이다.
> 예전 주석이 "그래서 모아서 보낸다"고 결론지은 것이 과했다. 모으지 않고도 흘려보낼 수 있다.

사용자가 창을 닫으면 `BrokenPipeError`가 나므로 조용히 삼킨다.

### 18.3 대화 기록 초기화 버튼

챗봇 패널 헤더에 `🗑 대화 지우기`를 넣었다.

- **말풍선만 지우지 않고 세션도 새로 판다.** 서버가 `session_id`로 턴 수를 세고
  (`SESSION_TURN_LIMIT`) 이력도 그 기준으로 붙는다. 세션을 그대로 두면 "지웠는데 왜 앞
  얘기를 기억하죠?"가 된다.
- 보낸 기록이 있을 때만 보인다(빈 창에서는 할 일이 없다).
- **응답을 받는 중에는 감춘다** — 답이 도착할 자리를 없애면 안 된다.

검증(로컬, API는 가짜 SSE로 대체):

| 상태 | 버튼 | 말풍선 |
|---|---|---|
| 빈 창 | 없음 | 0 |
| 질문 1건 보낸 뒤 | 보임 | 2 |
| 지운 뒤 | 없음 | 0 (안내문으로 복귀) · 입력창도 비움 |
