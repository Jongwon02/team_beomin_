...
- 품종 도구 응답의 '역병' 항목만으로 답합니다. 판정이 '자료 없음'이면 **위험이 없다는
  ...
  단정하지 말고, 자료가 없다는 사실과 작물 공통 예방법을 말하세요.
- 위험도가 있는 품종(예: 감자 수미 '높음')은 그 등급과 대처법을 함께 말합니다.
- 역병과 다른 병을 섞지 않습니다. 상추의 노균병·무름병·뿌리썩음병, 배의 검은무늬병은
  역병(Phytophthora)과 병원균이 다릅니다. 도구가 그 사실을 '비고'로 알려주면 그대로
  전달하세요.
- 약제는 성분명·상품명·희석배수를 말하지 않고 "등록 약제를 농약안전정보시스템에서
  확인하고 안전사용기준을 지키세요"로 안내합니다.
...
def tool_get_cultivar_candidates(crop, region, experience=None):
    ...
    for r in (d.get("ranking") or [])[:3]:
        ...
        item = {
            ...
            # pros/cons 가 '왜 이 품종인가 / 왜 아닌가'를 담는다. 예전에는 reasons·cautions·
            # variety_warnings 를 각각 실었는데 세 목록이 같은 말을 나눠 갖고 있어
            # 토큰만 먹고 답변이 산만해졌다. cons 안에 역병·병해·주의사항이 이미 합쳐져 있다.
            "추천이유": [p.get("text") for p in (r.get("pros") or [])[:2]],
            "고려할점": [c.get("text") for c in (r.get("cons") or [])[:3]],
        }
        if r.get("blockers"):
            item["막는요인"] = r["blockers"]
        # 역병은 따로 한 번 더 싣는다 - 사용자가 병해를 직접 물었을 때 대처법을 바로
        # 말할 수 있어야 하고, 위험도가 '조사된' 품종과 '자료 없는' 품종을 구분해야 한다.
        lb = r.get("late_blight") or {}
        if lb:
            item["역병"] = {"판정": lb.get("badge_text"), "설명": lb.get("status_text")}
            if lb.get("management"):
                item["역병"]["대처"] = lb["management"]
